#!/usr/bin/env python3
"""
Benchmark inference script adapted for 6 single-GPU instances.
GPUs 6-11, ports 9101-9106, 4 concurrent per server = 24 total concurrent.

Usage:
    python scripts/run_benchmark_inference_6gpu.py \
        --model-path ckpt/baseline_v6 \
        --model-id baseline_v6 \
        --output-dir results/v6/baseline_v6 \
        --reasoning
"""
...
LOG_DIR = Path("/tmp/vllm_logs_benchmark_6gpu")

PORTS = [9101, 9102, 9103, 9104, 9105, 9106]
GPU_SINGLES = [6, 7, 8, 9, 10, 11]  # 1 GPU per instance

N_INSTANCES = 6
GPUS_PER_INSTANCE = 1
CONCURRENCY_PER_SERVER = 4  # 4 × 6 = 24 total concurrent
...
def start_vllm_servers(model_path: str, reasoning: bool) -> list:
    """Start 6 vLLM server instances (1 GPU each). Returns list of (port, proc) tuples."""
...
def verify_gpus_freed():
    """Print GPU memory status for GPUs 6-11 after stopping servers."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Run benchmark inference across 4 chart benchmarks (6 single-GPU instances)")
    parser.add_argument("--model-path", required=True)
    parser.add_argument("--model-id", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--reasoning", action="store_true",
                        help="Enable reasoning mode (--reasoning-parser deepseek_r1)")
    ...
    log("=" * 60)
    log(f"  ChartVCR Benchmark Inference (6-GPU)")
