...
def reward_chartvr_v3(completions, answer, csv_path="", question="",
                      sigma=0.10, w_acc=0.5, w_proc=0.3, w_fmt=0.2,
                      verifier=None, **kwargs):
    """ChartVCR V3: V1 process scoring (continuous) + length penalty.
    V1 학습 signal 유지 + verbosity 억제."""
    if isinstance(answer, str):
        answer = [answer] * len(completions)
    if isinstance(csv_path, str):
        csv_path = [csv_path] * len(completions)
    ...
    # Batch verifier calls (same as V1)
    extractions = [None] * len(resps)
    if verifier:
        async def batch_verify():
            sem = asyncio.Semaphore(4)
    ...
    for i, (resp, gold, csv, q) in enumerate(zip(resps, answer, csv_path, question)):
        ...
        # R_format: 태그만 체크 (line count 보너스 제거)
        r_fmt = 0.0
        if '<think>' in resp and '</think>' in resp:
            r_fmt += 0.5
        if '<answer>' in resp:
        ...
        # Length penalty: 250 words 이하 → 0, 250-500 → 선형, 500+ → 0.3
        think_match = re.search(r'<think>(.*?)</think>', resp, re.DOTALL)
        if think_match:
            ...
            if think_len <= 250:
                ...
                length_penalty = min((think_len - 250) / 250, 1.0) * 0.3
        else:
            length_penalty = 0.0
...
def main():
    ...
    parser.add_argument("--reward_type", choices=["outcome_only", "chartvr", "chartvr_v2", "chartvr_v3", "geval"], required=True)
    ...
    verifier = None
    if args.reward_type in ("chartvr", "chartvr_v2", "chartvr_v3", "geval"):
    ...
    elif args.reward_type == "chartvr_v3":
        def reward_fn(completions, answer, csv_path="", question="", **kw):
            return reward_chartvr_v3(
                completions, answer, csv_path, question,
                sigma=args.sigma,
                w_proc=args.w_process,
                w_acc=0.5 + (0.3 - args.w_process),
    ...
    num_train_gpus = int(os.environ.get("NUM_TRAIN_GPUS", "6"))
